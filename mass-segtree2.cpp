...
const node NEUTRAL = node(0);
 
node unite(const node& a, const node& b) {
    return node(a.elem + b.elem);
}
...
void push(ll x, ll lx, ll rx) {
    if (lx == rx - 1 || tree[x].psh == -1) return;
    ll m = (lx + rx) / 2;
    tree[x * 2 + 1].elem = (m - lx) * tree[x].psh;
    tree[x * 2 + 1].psh = tree[x].psh;
    tree[x * 2 + 2].elem = (rx - m) * tree[x].psh;
